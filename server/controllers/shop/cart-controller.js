...
// when we add identical product 
   const findCurrentProductIndex = cart.items.findIndex(item => item.productId.toString() === productId)// productId is stored as String so we are converting it into string
   if(findCurrentProductIndex === -1){ // when we dont get index findindex return -1
...
module.exports = {
    ...
    fetchCartItem: tryCatchSimple(fetchCartItem)
}